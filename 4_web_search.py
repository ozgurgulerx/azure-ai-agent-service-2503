...
# Check if messages has a data property
if hasattr(messages, 'data'):
    response_message = next((msg for msg in messages.data if msg.role == MessageRole.AGENT), None)
    ...
    if response_message:
        ...
        print("No agent messages found in the thread.")
else:
    print(f"Unexpected response format from list_messages: {type(messages)}")
